...
class ContactsMigrate(MigrateAbstract):
    ...
    def _config(self):
        ...
        for functionid, function_name in cur.fetchall():
            ...
            self.function_list[functionid] = function_mdl.objects.create(name=function_name)
        ...
        for structuretypeid, structuretype_name in cur.fetchall():
            ...
            self.structuretype_list[structuretypeid] = structuretype_mdl.objects.create(name=structuretype_name)
    ...
    def _structure(self):
        ...
        def assign_abstact_values(new_legalentity, abstract_id):
            ...
            old_image_filename = join(self.old_db.tmp_path, "usr", "org_lucterios_contacts", "Image_%d.jpg" % abstract_id)
        ...
        for legalentityid, legalentity_super, legalentity_name, legalentity_type, legalentity_siren in cur.fetchall():
            ...
            if legalentity_type in self.structuretype_list.keys():
                new_legalentity.structure_type = self.structuretype_list[legalentity_type]
    ...
    def _relations(self):
        ...
        for physique, morale in cur.fetchall():
            if (morale in self.legalentity_list.keys()) and (physique in self.individual_list.keys()):
                new_resp = responsability_mdl.objects.create(individual=self.individual_list[physique], legal_entity=self.legalentity_list[morale])
                ids = []
                ...
                self.print_log("=> Responsability %s %s =%s", (six.text_type(self.individual_list[physique]), six.text_type(self.legalentity_list[morale]), six.text_type(ids)))
